	...
	IosSelect.prototype = {
		...
		initLayer: function() {
			...
			if (this.options.headerHeight && this.options.itemHeight) {
				...
				this.coverArea1Dom.style.top = this.headerHeight + this.itemHeight * 3 + 'rem';

				this.coverArea2Dom = document.querySelector('.cover-area2');
				this.coverArea2Dom.style.top = this.headerHeight + this.itemHeight * 4 + 'rem';
			}
			...
			this.iosSelectLayer.el.querySelector('.layer').style.height = this.itemHeight * 7 + this.headerHeight + 'rem';
			...
			this.scrollOne.on('scroll', function() {
				var pa = Math.abs(this.y/window.adaptive.newBase) / self.itemHeight;
				var plast = 1;
			...
			if (this.level >= 2) {
				this.twoLevelContainDom.style.height = this.itemHeight * 7 + 'rem';
			...
			if (this.level === 3) {
				this.threeLevelContainDom.style.height = this.itemHeight * 7 + 'rem';
